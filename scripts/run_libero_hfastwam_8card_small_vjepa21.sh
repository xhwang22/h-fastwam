#!/usr/bin/env bash
# LIBERO 8-GPU H-FastWAM SMALL with V-JEPA 2.1 ViT-G/16 (2B) as the
# frozen visual encoder. The existing flow-matching video DiT is unchanged.
set -euo pipefail
...
export TORCH_EXTENSIONS_DIR="/tmp/torch_ext_8card_small_vjepa21_$$"
...
if [[ ! -f "${VJEPA21_CHECKPOINT}" ]]; then
  echo "[8card-small-vjepa21] ERROR: missing ${VJEPA21_CHECKPOINT}" >&2
...
if [[ ! -f "${VJEPA21_REPO}/app/vjepa_2_1/models/vision_transformer.py" ]]; then
  echo "[8card-small-vjepa21] ERROR: V-JEPA 2.1 source tree missing at ${VJEPA21_REPO}." >&2
  exit 1
fi
...
RUN_NAME="${RUN_NAME:-libero_hfastwam_8card_small_vjepa21_ds}"
...
if [[ "${WANDB:-1}" == "1" ]]; then
  if [[ -z "${WANDB_API_KEY:-}" ]]; then
    echo "[8card-small-vjepa21] ERROR: set WANDB_API_KEY/~/.wandb_key or use WANDB=0." >&2
    exit 1
  fi
...
echo "[8card-small-vjepa21] model=${MODEL_CONFIG} (V-JEPA 2.1 ViT-G/16 2B, raw 1664-d features + Wan DiT)"
echo "[8card-small-vjepa21] global_batch=${GLOBAL_BATCH_SIZE} world_size=${WORLD_SIZE} batch_size=${BATCH_SIZE} grad_accum=${GRADIENT_ACCUMULATION_STEPS}"
echo "[8card-small-vjepa21] checkpoint=${VJEPA21_CHECKPOINT}"
echo "[8card-small-vjepa21] master=${MASTER_ADDR}:${MASTER_PORT} log=${LOG_FILE}"
