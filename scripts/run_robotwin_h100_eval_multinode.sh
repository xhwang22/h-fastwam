...
OUTPUT_TAG="${OUTPUT_TAG:-${EVAL_LABEL}_${CKPT_NAME}_h100_${MODE}_aligned_v2_${NODE_COUNT}node}"
...
export MODEL_KIND EVAL_LABEL RUN_DIR CKPT MODE NUM_GPUS OUTPUT_TAG EVAL_CONFIG
...
FORWARD_VARS=(
  MODEL_KIND EVAL_LABEL RUN_DIR CKPT MODE NUM_GPUS OUTPUT_TAG EVAL_CONFIG
  FASTWAM_EVAL_ENV CONDA_SH MAX_TASKS_PER_GPU SKIP_EVAL_PREPARE CHECK_ENV
  FASTWAM_EVAL_USE_CURRENT_ENV PYTHON_BIN NVIDIA_GRAPHICS_ENV
  ...
  DINOV3_MODEL_PATH SIGLIP2_MODEL_PATH
...
if [[ "${SKIP_MERGE:-0}" != "1" ]]; then
  ...
  MERGE_PYTHON="${PYTHON_BIN:-$(command -v python3 || command -v python)}"
  "${MERGE_PYTHON}" scripts/merge_robotwin_eval_shards.py \
