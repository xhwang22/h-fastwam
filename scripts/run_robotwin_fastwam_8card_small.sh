#!/usr/bin/env bash
# 8-GPU DDP (NON-DeepSpeed) run for FastWAM — CONTROL for hfastwam_small (VAE) — RoboTwin 2.0.
#
...
export TORCH_EXTENSIONS_DIR="/tmp/torch_ext_robotwin_8card_small_fastwam_$$"
...
if (( GLOBAL_BATCH_SIZE % _BATCH_DENOM != 0 )); then
  echo "[robotwin-small-fastwam] ERROR: GLOBAL_BATCH_SIZE=${GLOBAL_BATCH_SIZE} not divisible by world_size*grad_accum=${_BATCH_DENOM} (nproc=${NPROC_PER_NODE} nnodes=${NNODES} grad_accum=${GRADIENT_ACCUMULATION_STEPS})." >&2
  exit 1
fi
BATCH_SIZE=$(( GLOBAL_BATCH_SIZE / _BATCH_DENOM ))

RUN_NAME="${RUN_NAME:-robotwin_fastwam_8card_small_ds}"
LOG_DIR="${REPO_ROOT}/runs/robotwin_fastwam/${RUN_NAME}"
mkdir -p "${LOG_DIR}"
...
if [[ "${WANDB:-1}" == "1" ]]; then
  if [[ -z "${WANDB_API_KEY:-}" ]]; then
    echo "[robotwin-small-fastwam] ERROR: wandb enabled but no API key (set ~/.wandb_key or export WANDB_API_KEY, or pass WANDB=0)." >&2
    exit 1
  fi
...
echo "[robotwin-small-fastwam] CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES} nproc=${NPROC_PER_NODE} grad_accum=${GRADIENT_ACCUMULATION_STEPS} NO_CKPT=${NO_CKPT:-0} PROFILE_STEPS=${FASTWAM_PROFILE_STEPS}"
echo "[robotwin-small-fastwam] DeepSpeed ZeRO-2 via ${ACCEL_CONFIG} (global_batch=${GLOBAL_BATCH_SIZE} = world_size ${WORLD_SIZE} * batch_size ${BATCH_SIZE} * grad_accum ${GRADIENT_ACCUMULATION_STEPS})"
echo "[robotwin-small-fastwam] model=fastwam aligned to hfastwam_small (2048/16/28 random-init, VAE-48d, detach_video_for_action=${DETACH_VIDEO:-false})"
echo "[robotwin-small-fastwam] master=${MASTER_ADDR}:${MASTER_PORT}"
echo "[robotwin-small-fastwam] log=${LOG_FILE}"
