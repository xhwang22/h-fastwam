...
_multinode_dispatch() {
  ...
  for v in RUN_NAME GRADIENT_ACCUMULATION_STEPS LIBERO_DATA_ROOT \
            ...
            STANDARDISE_OUTPUT FRAME_GAP DETACH_VIDEO \
            ACTION_DIT_PRETRAINED_PATH FASTWAM_USE_ZERO_REDUNDANCY_OPTIMIZER; do
    [[ -n "${!v+x}" ]] && kvs+=("${v}=${!v}")
  done
