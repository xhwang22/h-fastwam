...
_multinode_dispatch() {
  ...
  for v in RUN_NAME MODEL_CONFIG LAUNCH_LABEL GLOBAL_BATCH_SIZE PER_DEVICE_BATCH_SIZE \
            GRADIENT_ACCUMULATION_STEPS SAVE_EVERY LIBERO_DATA_ROOT ROBOTWIN_DATA_ROOT \
            NO_CKPT FASTWAM_PROFILE_STEPS CUDA_VISIBLE_DEVICES \
            WANDB WANDB_API_KEY WANDB_PROJECT WANDB_GROUP \
            WANDB_MODE WANDB_ENTITY \
            NCCL_SOCKET_IFNAME NCCL_IB_DISABLE NCCL_NET_GDR_LEVEL \
            ...
            STANDARDISE_OUTPUT TEMPORAL_DOWNSAMPLE FRAME_GAP DETACH_VIDEO \
            VIDEO_LATENT_CACHE_DIR LATENT_CACHE_SHARD_SIZE LATENT_CACHE_BATCH_SIZE \
