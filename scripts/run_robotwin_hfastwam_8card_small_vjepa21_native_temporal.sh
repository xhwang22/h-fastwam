#!/usr/bin/env bash
# RoboTwin V-JEPA 2.1 Flow DiT native temporal resolution: retain 5 tubelets.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
export MODEL_CONFIG="${MODEL_CONFIG:-hfastwam_small_vjepa21}"
export RUN_NAME="${RUN_NAME:-robotwin_hfastwam_8card_small_vjepa21_native_t5_ds}"
...
export VIDEO_LATENT_CACHE_DIR="${VIDEO_LATENT_CACHE_DIR:-${VIDEO_LATENT_CACHE_ROOT:-${REPO_ROOT}/data/video_latent_cache}/robotwin/vjepa21_native_t5_raw}"
