...
# as the frozen visual encoder and the existing flow-matching video DiT.
set -euo pipefail
...
export TORCH_EXTENSIONS_DIR="/tmp/torch_ext_robotwin_8card_small_vjepa21_$$"
...
if [[ ! -f "${VJEPA21_CHECKPOINT}" ]]; then
  echo "[robotwin-small-vjepa21] ERROR: missing ${VJEPA21_CHECKPOINT}" >&2
...
if [[ ! -f "${VJEPA21_REPO}/app/vjepa_2_1/models/vision_transformer.py" ]]; then
  echo "[robotwin-small-vjepa21] ERROR: V-JEPA 2.1 source tree missing at ${VJEPA21_REPO}." >&2
  exit 1
fi
...
if [[ "${WANDB:-1}" == "1" ]]; then
  if [[ -z "${WANDB_API_KEY:-}" ]]; then
    echo "[robotwin-small-vjepa21] ERROR: set WANDB_API_KEY/~/.wandb_key or use WANDB=0." >&2
    exit 1
  fi
