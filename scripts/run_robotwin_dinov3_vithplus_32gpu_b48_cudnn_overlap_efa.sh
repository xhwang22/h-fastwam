#!/usr/bin/env bash
# 32-GPU DINOv3 ViT-H+/16 840M + Flow-DiT on AWS HyperPod.
set -euo pipefail
...
(
  ...
  if [[ ! -f "${DINOV3_MODEL_PATH}/config.json" || ! -f "${DINOV3_MODEL_PATH}/model.safetensors" ]]; then
    if [[ -z "${HF_TOKEN:-${HUGGING_FACE_HUB_TOKEN:-}}" ]]; then
      echo "[aws-dinov3-vithplus] ERROR: DINOv3 is gated; set HF_TOKEN or pre-download ${DINOV3_MODEL_PATH}." >&2
      exit 2
    fi
    echo "[aws-dinov3-vithplus] downloading facebook/dinov3-vith16plus-pretrain-lvd1689m to ${DINOV3_MODEL_PATH}"
...
export MODEL_CONFIG=hfastwam_small_dinov3_vithplus
...
if [[ ! -f "${ROBOTWIN_WEBDATASET_ROOT}/dataset.done" ]]; then
  echo "[aws-dinov3-vithplus] ERROR: completed WebDataset not found: ${ROBOTWIN_WEBDATASET_ROOT}/dataset.done" >&2
  echo "Set ROBOTWIN_WEBDATASET_ROOT to the completed indexed RoboTwin dataset." >&2
  exit 2
fi
export LOG_ROOT="${LOG_ROOT:-/efs/shaunxhwang}"
export RUN_NAME="${RUN_NAME:-robotwin_dinov3_vithplus_840m_32gpu_b48_cudnn_overlap_efa}"
export WANDB_PROJECT="${WANDB_PROJECT:-fastwam-robotwin-encoder-ablation}"
export WANDB_GROUP="${WANDB_GROUP:-dinov3-vithplus-flow-dit}"
export LAUNCH_LABEL=aws-dinov3-vithplus-840m
export VISUAL_ENCODER_DESCRIPTION="DINOv3 ViT-H+/16 840M, raw 1280-d features + Wan DiT"
