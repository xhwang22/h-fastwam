...
# HyperPod injects the allocation's 4-node PET topology into every container.
# This launcher intentionally uses only the current node, so remove every
# node-level distributed hint before the shared AWS setup resolves topology.
