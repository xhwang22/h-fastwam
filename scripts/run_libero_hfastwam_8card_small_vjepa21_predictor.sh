#!/usr/bin/env bash
# LIBERO 8-GPU H-FastWAM SMALL with V-JEPA 2.1 ViT-G/16 (2B) and the
# deterministic JEPA predictor replacing the flow-matching video DiT.
set -euo pipefail
...
export TORCH_EXTENSIONS_DIR="/tmp/torch_ext_8card_small_vjepa21_predictor_$$"
...
if [[ ! -f "${VJEPA21_CHECKPOINT}" ]]; then
  echo "[8card-small-vjepa21-predictor] ERROR: missing ${VJEPA21_CHECKPOINT}" >&2
...
if [[ ! -f "${VJEPA21_REPO}/app/vjepa_2_1/models/vision_transformer.py" ]]; then
  echo "[8card-small-vjepa21-predictor] ERROR: V-JEPA 2.1 source tree missing at ${VJEPA21_REPO}." >&2
  exit 1
fi
...
RUN_NAME="${RUN_NAME:-libero_hfastwam_8card_small_vjepa21_predictor_ds}"
...
if [[ "${WANDB:-1}" == "1" ]]; then
  if [[ -z "${WANDB_API_KEY:-}" ]]; then
    echo "[8card-small-vjepa21-predictor] ERROR: set WANDB_API_KEY/~/.wandb_key or use WANDB=0." >&2
    exit 1
  fi
...
echo "[8card-small-vjepa21-predictor] model=hfastwam_small_vjepa21_predictor (V-JEPA 2.1 ViT-G/16 2B + random-init JEPA predictor, 1664-d L1 target)"
echo "[8card-small-vjepa21-predictor] global_batch=${GLOBAL_BATCH_SIZE} world_size=${WORLD_SIZE} batch_size=${BATCH_SIZE} grad_accum=${GRADIENT_ACCUMULATION_STEPS}"
echo "[8card-small-vjepa21-predictor] checkpoint=${VJEPA21_CHECKPOINT}"
echo "[8card-small-vjepa21-predictor] master=${MASTER_ADDR}:${MASTER_PORT} log=${LOG_FILE}"
