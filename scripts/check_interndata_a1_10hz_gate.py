#!/usr/bin/env python3
"""Validate InternData-A1 v5 manifest and canonical 10Hz samples."""
...
from fastwam.datasets.lerobot.intern_a1_v3_dataset import (
    ACTION_NATIVE_INDICES,
    InternDataA1V3Dataset,
    NATIVE_WINDOW_FRAMES,
    STATE_NATIVE_INDICES,
    TARGET_CONTROL_HZ,
...
def _family_summary(
    ...
    for family in ("dual", "single"):
        ...
        summary[family] = {
            ...
            "clip_starts": int(
                np.maximum(lengths[mask].astype(np.int64) - 96, 0).sum()
            ),
            "robot_types": sorted(
                {
...
def main() -> None:
    ...
    if TARGET_CONTROL_HZ != 10 or NATIVE_WINDOW_FRAMES != 97:
        raise ValueError("InternData 10Hz constants are inconsistent.")
    if STATE_NATIVE_INDICES.tolist() != list(range(0, 96, 3)):
        raise ValueError("InternData state indices are not 10Hz aligned.")
    if ACTION_NATIVE_INDICES.tolist() != list(range(2, 96, 3)):
        raise ValueError("InternData action indices are not next-state aligned.")
    if VIDEO_NATIVE_INDICES.tolist() != list(range(0, 97, 12)):
        raise ValueError("InternData video indices are not 0.4s aligned.")
    ...
    for family in decoded:
        ...
        for episode_row in chosen:
            clip_count = int(dataset.arrays["length"][episode_row]) - 96
