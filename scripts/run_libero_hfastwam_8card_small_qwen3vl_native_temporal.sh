#!/usr/bin/env bash
# LIBERO Qwen3-VL native temporal resolution: retain all 5 video tubelets.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
export MODEL_CONFIG="${MODEL_CONFIG:-hfastwam_small_siglip2}"
export RUN_NAME="${RUN_NAME:-libero_hfastwam_8card_small_qwen3vl_native_t5_ds}"
export LAUNCH_LABEL="${LAUNCH_LABEL:-8card-small-qwen3vl-native-t5}"
...
export VIDEO_LATENT_CACHE_DIR="${VIDEO_LATENT_CACHE_DIR:-${VIDEO_LATENT_CACHE_ROOT:-${REPO_ROOT}/data/video_latent_cache}/libero/qwen3vl_native_t5_raw}"
