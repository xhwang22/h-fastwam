#!/usr/bin/env bash
# LIBERO 8-GPU H-FastWAM SMALL with the frozen Qwen3-VL-2B visual tower.
# The visual tower replaces DINO/VAE and emits raw 1024-d features to the
# existing flow-matching video DiT. It is not connected to the language expert.
set -euo pipefail
...
export TORCH_EXTENSIONS_DIR="/tmp/torch_ext_8card_small_siglip2_$$"
...
LAUNCH_LABEL="${LAUNCH_LABEL:-8card-small-siglip2}"
...
RUN_NAME="${RUN_NAME:-libero_hfastwam_8card_small_siglip2_ds}"
