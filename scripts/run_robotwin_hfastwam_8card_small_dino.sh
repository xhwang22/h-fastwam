...
# model=hfastwam_small_dino: the video & action experts are shrunk and aligned to the
...
#     bash scripts/run_libero_hfastwam_8card_small_dino.sh
set -euo pipefail
...
# --- corporate HTTP proxy (lets HF Hub model downloads reach the internet) ---
# Only affects HTTP(S) traffic; NCCL/GLOO/torchrun use raw TCP and are unaffected
# (cluster-internal IPs are excluded via no_proxy). Set NO_PROXY_OVERRIDE / disable
# by exporting HTTP_PROXY="" before launch if your node already has direct egress.
...
# --- HuggingFace token (DINOv3 is a gated repo) ---
# Read from ~/.hf_token (chmod 600, NOT in repo / git / logs). Override by
# exporting HF_TOKEN before launch.
...
# --- offline / fast init ---
# HF_HOME points at the in-repo (shared-FS) cache holding the pre-downloaded
# Qwen3-VL-2B weights, so language_local_files_only=true resolves offline on
# every node. DINOv3 loads from an explicit local path in the model config.
...
# (HF_HOME lives on cephfs for the model weights, but datasets must stay local).
export HF_DATASETS_CACHE="${HF_DATASETS_CACHE:-${HOME}/.cache/huggingface/datasets}"
export HF_DATASETS_OFFLINE=1
export PYTHONDONTWRITEBYTECODE=1
export TORCH_EXTENSIONS_DIR="/tmp/torch_ext_robotwin_8card_small_dino_$$"
...
if (( GLOBAL_BATCH_SIZE % _BATCH_DENOM != 0 )); then
  echo "[robotwin-small-dino] ERROR: GLOBAL_BATCH_SIZE=${GLOBAL_BATCH_SIZE} not divisible by world_size*grad_accum=${_BATCH_DENOM} (nproc=${NPROC_PER_NODE} nnodes=${NNODES} grad_accum=${GRADIENT_ACCUMULATION_STEPS})." >&2
  exit 1
fi
BATCH_SIZE=$(( GLOBAL_BATCH_SIZE / _BATCH_DENOM ))

RUN_NAME="${RUN_NAME:-robotwin_hfastwam_8card_small_dino_ds}"
...
if [[ "${WANDB:-1}" == "1" ]]; then
  if [[ -z "${WANDB_API_KEY:-}" ]]; then
    echo "[robotwin-small-dino] ERROR: wandb enabled but no API key (set ~/.wandb_key or export WANDB_API_KEY, or pass WANDB=0)." >&2
    exit 1
  fi
...
# DINO latent standardisation toggle.
# The DINOEncoder default `standardise_output=true` does PER-BATCH channel
# standardisation (mean/var over B,T,H,W). At single-frame inference (T=1) the
# batch statistics differ wildly from multi-frame training (T_lat≈9), so the
# standardised DINO features no longer match training -> train/infer mismatch.
# Set STANDARDISE_OUTPUT=false to emit RAW DINO features (no batch standardise),
# which is consistent between training and single-frame inference.
...
echo "[robotwin-small-dino] CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES} nproc=${NPROC_PER_NODE} grad_accum=${GRADIENT_ACCUMULATION_STEPS} NO_CKPT=${NO_CKPT:-0} PROFILE_STEPS=${FASTWAM_PROFILE_STEPS}"
echo "[robotwin-small-dino] DeepSpeed ZeRO-2 via ${ACCEL_CONFIG} (global_batch=${GLOBAL_BATCH_SIZE} = world_size ${WORLD_SIZE} * batch_size ${BATCH_SIZE} * grad_accum ${GRADIENT_ACCUMULATION_STEPS})"
echo "[robotwin-small-dino] model=hfastwam_small_dino (small experts 2048/16/28 random-init + frozen DINOv3 ViT-S encoder, in/out_dim=384)"
echo "[robotwin-small-dino] master=${MASTER_ADDR}:${MASTER_PORT}"
echo "[robotwin-small-dino] log=${LOG_FILE}"
