#!/usr/bin/env bash
# 32-GPU native SigLIP2 So400M + Flow-DiT on AWS HyperPod.
set -euo pipefail
...
(
  ...
  if [[ ! -f "${SIGLIP2_MODEL_PATH}/config.json" || ! -f "${SIGLIP2_MODEL_PATH}/model.safetensors" ]]; then
    echo "[aws-siglip2] downloading google/siglip2-so400m-patch16-384 to ${SIGLIP2_MODEL_PATH}"
...
export LAUNCH_LABEL=aws-native-siglip2-so400m
