#!/usr/bin/env bash
# 32-GPU V-JEPA21 IDM: per-GPU batch 24 x grad-accum 2 = global batch 1536.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

export FASTWAM_EXPECTED_WORLD_SIZE=32
export GLOBAL_BATCH_SIZE=$(( 24 * FASTWAM_EXPECTED_WORLD_SIZE * 2 ))
...
export RUN_NAME="${RUN_NAME:-robotwin_vjepa21_predictor_idm_causal_tubelet_32gpu_b24_acc2_cudnn_overlap_efa}"
